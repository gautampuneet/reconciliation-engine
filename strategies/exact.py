...
class ExactMatchStrategy(MatchingStrategy):
    def _build_key(self, transaction_id: str, currency: str, amount: Decimal) -> tuple[str, str, Decimal]:
        return (transaction_id, currency, amount)
    ...
    def match(
        ...
        index: dict[tuple[str, str, Decimal], list[LedgerEntry]] = defaultdict(list)
        for ledger in ledger_entries:
            key = self._build_key(ledger.transaction_id, ledger.currency, ledger.amount)
        ...
        for transaction in transactions:
            key = self._build_key(transaction.transaction_id, transaction.currency, transaction.amount)
